...
class Login(HttpClient):
    ...
    def __init__(self, vpath, qq=0):
        ...
        APPID = getReValue(str(html), r'<input type="hidden" name="aid" value="(\d+)" />', 'Get AppId Error', 1)
        logging.critical("正在获取login_sig")
        sign = getReValue(str(html), r'g_login_sig\s*=\s*encodeURIComponent\s*\("(.*?)"\)', 'Get Login Sign Error', 0)
        logging.info('get sign : %s', sign)
        logging.critical("正在获取pt_version")
        JsVer = getReValue(str(html), r'g_pt_version\s*=\s*encodeURIComponent\s*\("(\d+)"\)', 'Get g_pt_version Error', 1)
        logging.info('get g_pt_version : %s', JsVer)
        logging.critical("正在获取mibao_css")
        MiBaoCss = getReValue(str(html), r'g_mibao_css\s*=\s*encodeURIComponent\s*\("(.*?)"\)', 'Get g_mibao_css Error', 1)
        ...
        while True:
            ...
            while True:
                html = self.Get('https://ssl.ptlogin2.qq.com/ptqrlogin?ptqrtoken={0}&webqq_type=10&remember_uin=1&login2qq=1&aid={1}&u1=http%3A%2F%2Fw.qq.com%2Fproxy.html%3Flogin2qq%3D1%26webqq_type%3D10&ptredirect=0&ptlang=2052&daid=164&from_ui=1&pttype=1&dumy=&fp=loginerroralert&action=0-0-{2}&mibao_css={3}&t=1&g=1&js_type=0&js_ver={4}&login_sig={5}&pt_randsalt=2'.format(getQRtoken(QRSig),APPID, date_to_millis(datetime.datetime.utcnow()) - StarTime, MiBaoCss, JsVer, sign),
                        SmartQQUrl)
                #logging.info(html)
                ret = html.split(bytearray("'",'utf-8'))
                logging.info(ret[1])
                logging.info(type('0'))
                if ret[1] == bytearray('65','utf-8') or ret[1] == bytearray('0','utf-8'):  # 65: QRCode 失效, 0: 验证成功, 66: 未失效, 67: 验证中
                    break
                time.sleep(2)
            if ret[1] == bytearray('0','utf-8') or T > self.MaxTryTime:
                break

        logging.info(ret)
        if ret[1] != bytearray('0','utf-8'):
        ...
        # 记录登陆账号的昵称
        tmpUserName = ret[11]
        html = self.Get(ret[5].decode('utf-8'))

        url = getReValue(html.decode('utf-8'), r' src="(.+?)"', 'Get mibao_res Url Error.', 0)
        if url != '':
