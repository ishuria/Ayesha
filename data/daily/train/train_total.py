...
def daily_train_lstm(code_list,batch_size,time_step,term,date,cursor):
        model_path = '/home/ayesha/data/models/30'
        ...
        for code in code_list:
            weight = {
             'in':tf.Variable(tf.random_normal([input_size,rnn_unit]),name=code+'weight_in'),
             'out':tf.Variable(tf.random_normal([rnn_unit,1]),name=code+'weight_out')
            }
            biase = {
                'in':tf.Variable(tf.constant(0.1,shape=[rnn_unit,]),name=code+'biase_in'),
                'out':tf.Variable(tf.constant(0.1,shape=[1,]),name=code+'biase_out')
               }
