...
#获取训练集
def get_train_data(code,train_begin_year='2010',train_end_year='2014'):
    ...
    for normalized_stock_history in normalized_stock_history_list:
        train_x.append(normalized_stock_history.tolist())
    for normalized_stock_price in normalized_stock_price_list:
        train_y.append(normalized_stock_price.tolist())
...
weights={
         'in':tf.Variable(tf.random_normal([INPUT_NODE,rnn_unit])),
...
def lstm(X):     
    ...
    w_in=weights['in']
    b_in=biases['in']  
    input=tf.reshape(X,[-1,INPUT_NODE])  #需要将tensor转成2维进行计算，计算后的结果作为隐藏层的输入
...
#——————————————————训练模型——————————————————
def train_lstm(code,train_begin_year='2010',train_end_year='2014'):
    X=tf.placeholder(tf.float32, shape=[None,INPUT_NODE])
    Y=tf.placeholder(tf.float32, shape=[None,OUTPUT_NODE])
    get_train_data(code,train_begin_year,train_end_year)
    pred,_=lstm(X)
    #损失函数
    ...
    with tf.Session() as sess:
        ...
        #重复训练10000次
        for i in range(200):
            _,loss_=sess.run([train_op,loss],feed_dict={X:train_x[i],Y:train_y[i]})
            print(i,loss_)
            if i % 200==0:
