...
def get_test_data(code,time_step,term,date):
	...
	for i in range(sample_size-1):
		x = stock_history_arr[i*time_step:(i+1)*time_step,:5]
		test_x.append(x.tolist())

	x = stock_history_arr[-1 * time_step:,:5]
...
def predict_lstm(code,time_step,term,begin,end):
	with tf.variable_scope(code + '_' + term, reuse=None):
		...
		while begindate<=enddate:
			...
			test_x,future_price = get_test_data(code,time_step,term,date)
			if len(test_x) == 0:
...
if __name__ == '__main__':
	db_connect()
	#predict_lstm(code,time_step,term,begin,end):
	predict_lstm('600000',30,'30','2013-01-01','2017-12-01')
	'''
